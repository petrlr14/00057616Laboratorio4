...
void getData(istream &file, int nlines, int n, int mode, item *item_list) {
  string line;
  file >> line;
...
void updateConditionNodes(int n, condition *list, int delta) {
  for (int i = 0; i < n; i++) list[i].setNode1(list[i].getNode1() + delta);
}

void joinConditions(condition *list, int n1, int n2, condition *list1,
                    condition *list2) {
  int i;
  for (i = 0; i < n1; i++) list[i] = list1[i];
  for (int j = 0; j < n2; j++) {
    list[i] = list2[j];
    i++;
  }
...
void readMeshAndConditions(mesh &m) {
  ...
  do {
    cout
        << "Ingrese el nombre del archivo que contiene los datos de la malla: ";
    cin >> filename;
    file.open(filename);
  } while (!file);
  ...
  getData(file, SINGLELINE, nnodes, INT_FLOAT, m.getNodes());
